...
// 함수 선언할 필요가 있을까?
// 함수 선언을 할 필요가 있다.
// 특정 임베디드 컴파일러는 무조건 함수 선언을 해야지 함수를 사용할 수 있게 되어 있다.
// 따라서 특정 컴파일러에서 함수선언을 하지 않고도 정상 동작했던 코드가 
// 비정상적(컴파일 에러, 런타임 에러)으로 동작할 수 있다.
// 또한 전체적인 프로그램의 구조를 이해하는 측면에서도 함수선언을 코드 제일앞에 해주는 것은
